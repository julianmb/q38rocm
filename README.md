...
Every turn after the first pays **seconds, not minutes** — and MTP speculative decoding stays enabled throughout (no `spec-boundary-mismatch` fallbacks, zero cold resets). Raw artifacts: `benchmarks/long_context_cache_20260831_*.json` + `...20260901_124944.json` (v1.6.0, 32K MMQ tuning: 199→118s, 40% faster).

> **v1.7.0 re-validation (2026-09-03, `75e67a9` org repo, ROCm0, `q8_0/q8_0`):** same harness, deeper wins — 32 K: 116.9 s @272 → **0.53 s (219.3×)** · 64 K: 303.8 s @212 → **0.72 s (423.8×)** · 130 K: 881.3 s @148 → **1.10 s (802.6×)**. Raw artifacts: `benchmarks/long_context_cache_20260903_*.json`. Note: v1.7.0 dropped the `turbo4` KV type, so these runs use `q8_0` V-cache; marker-retrieval passes need `--reasoning off` on thinking models (the harness default lets the model burn its budget on preamble). Backend note: prefill numbers on ROCm0 (stable under concurrent desktop iGPU load); decode on Vulkan0 reaches the 30+ tok/s MTP rates in the tables above when the iGPU is otherwise idle.

> **Structured profile (DFlash2) head-to-head at 32K** — same doc, same 1.1 GiB `Qwen3.8-27B-DFlash2-Q4_K_M.gguf` draft, Laurent `b10685` engine (Vulkan0, `q8_0` KV) vs `speed` (MTP, ROCm0, TurboQuant): `structured` cold 150.3 s @211 tok/s → cached **0.57 s (264×)** / **21.2 tok/s decode**, vs `speed` 118.9 s @267 tok/s → 5.11 s (23×) / 15.8 tok/s. Raw: `benchmarks/long_context_cache_20260901_201307.json`. Use `speed` for long-context, `structured` for JSON/code bursts. `--profile structured` requires the Laurent engine (`--spec-draft-adaptive`) and `--slots 1`; it fails closed on stock engines (measured degenerate output there: 6.7% acceptance, hung requests — bypass only with `DFLASH_ALLOW_STOCK=1` for testing).
